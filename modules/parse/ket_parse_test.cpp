 ...
 * @details 0、負数、最大値、最小値を入力し、符号付き整数へ完全消費で変換されることを確認。
 * @pre C++17以降。
 ...
 * @details
 * 空文字列、空白、部分消費、overflow、underflowを入力し、失敗時にoutが不変であることを確認。
 * @pre C++17以降。
 ...
 * @details 0、通常値、最大値を入力し、符号なし整数へ完全消費で変換されることを確認。
 * @pre C++17以降。
 * @post テスト対象APIと外部状態の変更なし。
 */
 ...
 * @details 空文字列、空白、部分消費、overflow、負数を入力し、失敗時にoutが不変であることを確認。
 * @pre C++17以降。
 ...
 * @details prefixなし、`0x`、`0X`、大文字hexを入力し、16進整数へ変換されることを確認。
 * @pre C++17以降。
 * @post テスト対象APIと外部状態の変更なし。
 */
 ...
 * @details
 * 空文字列、prefixのみ、不正hex、部分消費、空白、overflowを入力し、失敗時にoutが不変であることを確認。
 * @pre C++17以降。
...
TEST(KetParseTest, RejectsInvalidHexIntegers)
{
	...
	std::uint32_t partial_value = 77U;
	std::uint32_t whitespace_value = 77U;
	std::uint32_t overflow_value = 77U;
	...
	const auto whitespace_ok = ket::parse::TryHex(" ff", whitespace_value);
	...
	EXPECT_EQ(partial_value, 77U);
	EXPECT_EQ(whitespace_value, 77U);
	EXPECT_EQ(overflow_value, 77U);
	EXPECT_EQ(invalid_optional, std::nullopt);
}
 ...
 * @details `true`、`false`、`1`、`0`を入力し、case-sensitiveなbool値へ変換されることを確認。
 * @pre C++17以降。
 * @post テスト対象APIと外部状態の変更なし。
 */
