...
namespace ket
{
	...
	namespace detail
	{
		...
		{
			return RangesOverlap(destination.data(), destination.size(), part.data(), part.size());
		}
		 ...
		 * @pre `destination`は有効なstd::string。各要素はstd::string_viewへ変換可能、またはchar。
		 * @post 引数と外部状態の変更なし。
		 */
		template <typename... Parts>
	...
	std::string StrCat(const Parts&... parts)
	{
		static_assert((detail::kIsStringPart<Parts> && ...),
					  "ket::StrCat accepts char and types convertible to std::string_view only.");
	...
	void StrAppend(std::string& destination, const Parts&... parts)
	{
		static_assert(
			...
			"ket::StrAppend accepts char and types convertible to std::string_view only.");
		...
		if (has_alias)
		{
			const auto suffix = StrCat(parts...);
