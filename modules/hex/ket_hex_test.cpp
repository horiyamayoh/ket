 ...
 * @details 0、代表値、64bit最大値を入力し、最小幅zero paddingと大文字小文字が固定されることを確認。
 * @pre C++17以降。
 * @post テスト対象APIと外部状態の変更なし。
 */
TEST(KetHexTest, EncodesIntegerWithWidthAndCase)
{
	const auto zero = ket::hex::Encode(0U);
	const auto padded_lower = ket::hex::Encode(0x2aU, 4U, ket::hex::LetterCase::kLower);
	const auto default_upper = ket::hex::Encode(0xdeadbeefU);
	const auto maximum = ket::hex::Encode(UINT64_MAX, 16U, ket::hex::LetterCase::kLower);
 ...
 * @details 奇数桁、不正文字、separator付きhexを入力し、std::nulloptを返すことを確認。
 * @pre C++17以降。
 * @post テスト対象APIと外部状態の変更なし。
 */
 ...
 * @test
 * @brief hexdumpの空入力確認。
 * @details byte列overloadとvoid pointer overloadへ空入力を渡し、空文字列を返すことを確認。
 * @pre C++17以降。
 * @post テスト対象APIと外部状態の変更なし。
 */
TEST(KetHexTest, DumpsEmptyInputAsEmptyString)
{
	const std::uint8_t* const null_bytes = nullptr;
	const auto byte_dump = ket::hex::Dump(null_bytes, 0U);
	const void* const null_data = nullptr;
	const auto void_dump = ket::hex::Dump(null_data, 0U);

	EXPECT_EQ(byte_dump, std::string());
	EXPECT_EQ(void_dump, std::string());
}
