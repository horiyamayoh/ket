...
| `ranges`         | done     | `Existing`           | C++11   | C++11〜17 | C++20以降  | header-only + test               | algorithm   | index付き range 走査                     | `ket::ranges::ForEachWithIndex`, `ket::ranges::FindIndexIf`                                                   | not found は false/out不変、predicate例外伝播           | 実装済み境界値テスト                       | std algorithm 別名なし           |
