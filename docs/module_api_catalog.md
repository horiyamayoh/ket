...
| `platform`       | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | platform    | errno/Windows error の文字列化           | `ket::platform::FormatErrno`, `ket::platform::FormatWindowsError`, `ket::platform::ReadEnvironmentVariable`   | Windows API は `_WIN32` 限定、env missing は `std::nullopt` | known errno、missing/empty env、non-Windows smoke | Windows は UTF-8 変換            |
...
- Purpose: errno/Windows error/environment variable の小さい platform 補助。Existing module のため、公開詳細は
  header Doxygen と実装を正本とし、このカードは現状仕様と追加禁止範囲の要約。
- C++ version: 最小要件 C++17。推奨版 C++17以降。推奨理由:
...
- Behavior: `FormatErrno` は errno 番号を platform の thread-safe API で文字列化し、結果を
  ...
  environment と error message の wide API を使い、戻り値は UTF-8 narrow string。`ReadEnvironmentVariable` と
  `FormatWindowsError` は現在threadのlast-error codeを保持する。`FormatWindowsError` は Windows message
  resource由来の末尾CR、LF、tab、space、NULを除去する。Windows 専用 API は
...
- Failure/edge cases: unknown errno、missing env、empty env value、empty name、NUL を含む name、
  Windows env name/value UTF-8 変換失敗、Windows error message UTF-8 変換失敗。
  environment を他 thread が同時変更する場合の一貫性は platform API の規約に従う。Windows env 変換に失敗した
  場合は `std::nullopt`、Windows error message 変換に失敗した場合は ASCII fallback を返す。
- Complexity/performance: error message と environment value の長さ O(n) で string を1回確保する。
  Windows UTF-8 変換も出力長 O(n)。
- Tests: known errno が非空かつfallbackでないこと、unknown errno fallback、errno保存、missing env、present env、
  empty env value、empty name、NUL name、environment復元、非Windows smoke、Windows 環境では known/unknown
  Windows error message と last-error保存。environment test は一意な `KET_` prefix の変数名を使い、
