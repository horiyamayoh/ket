...
| `build_config`   | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | config      | feature detection                        | `KET_CXX_VERSION`, `KET_CXX_AT_LEAST`, `KET_HAS_STD_OPTIONAL`, `KET_OS_LINUX`                                 | macro は `KET_` prefix、値は 0/1                        | compiler/OS 条件、include順、C++11/20      | 他moduleの必須依存にしない       |
...
- Tests: C++11 compile-only、C++20 compile-only、macro が全て定義済み、0/1 値、
