...
| `tlv`            | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | binary      | length-prefix/TLV                        | `ket::tlv::Encode`, `ket::tlv::Append`, `ket::tlv::TryDecode`                                                 | type u16/length u32 BE、decode失敗は out不変            | 短い入力、length超過、roundtrip            | header は6 bytes                 |
...
- Public API Signatures（`namespace ket::tlv`）:
  - `struct View { std::uint16_t type = 0U; const std::uint8_t* value = nullptr; std::uint32_t value_size = 0U; };`
  - `struct DecodeResult { View view; std::size_t consumed = 0U; };`
...
- Behavior: wire format は `type:uint16 big-endian` + `length:uint32 big-endian` + `value bytes`。
  ...
  `Append` は入力 value が `dst` の内部 storage を指す場合でも、先に一時 record を構築してから `dst` へ追加する。
...
- Failure/edge cases: `value == nullptr && value_size == 0` は空 value。`value == nullptr && value_size > 0` は
  encode 側 precondition 違反。`TryDecode(nullptr, size, out)` は `size` に関わらず `false` で `out` 不変。
...
- Complexity/performance: encode/append は value_size O(n) で vector へ byte copy。decode は value を copy せず
  header を読むだけの O(1)。allocation は encode/append の vector 成長のみ。`Append` は record 構築または
  allocation 失敗時に `dst` を変更しない strong exception guarantee。
- Tests: DecodeResult default初期値、empty value、1 byte value、256 byte length、type 0/65535、
  Doxygen例、roundtrip、複数record先頭decode、self-overlap append、short header、short value、
  null decode input、null+非0 encode precondition Doxygen、length過大、big-endian golden bytes、
  max `uint32` length header、decode失敗時 out 不変、view lifetime は呼び出し側責任であることの Doxygen。
