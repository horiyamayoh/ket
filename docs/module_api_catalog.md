...
| ---------------- | -------- | -------------------- | ------- | --------- | ---------- | -------------------------------- | ----------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------------- | ------------------------------------------------- | ------------------------------------------ | -------------------------------- |
| `bcd`            | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | binary      | packed BCD 変換                     | `ket::bcd::ToInt`, `ket::bcd::FromInt8`, `ket::bcd::ToDecimalString`                                          | 不正nibble、空入力、overflow は失敗値             | 実装済み境界値テスト                       | 追加は BCD 妥当性判定まで        |
...
| `bits`           | P0       | `Ready`              | C++11   | C++11以降 | なし       | header-only + test               | numeric     | bit/nibble/mask の事故防止          | `ket::bits::HighNibble`, `ket::bits::LowNibble`, `ket::bits::HasBit`, `ket::bits::TryMask`                    | unsigned integral 限定、範囲外 bit は失敗/false   | nibble、bit幅、mask境界                    | C++20 `<bit>` は一部のみ重複     |
...
| `hex`            | P0       | `Ready`              | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | diagnostic  | bytes と16進文字列/hex dump         | `ket::hex::Encode`, `ket::hex::Decode`, `ket::hex::Dump`, `ket::hex::EncodeU64`                               | null+非0 size は precondition、奇数桁は失敗       | 空入力、separator、不正文字                | dump形式は固定                   |
...
| `byte_view`      | P1       | `Ready`              | C++11   | C++11〜17 | C++20以降  | header-only + test               | view        | non-owning byte span                | `ket::byte_view::View`, `MutableView`, `TrySubView`                                                           | `nullptr+0` は空、`nullptr+非0` は invalid        | lifetime、subview、bounds                  | method と free を併用            |
...
| `platform`       | P2       | `Ready`              | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | platform    | errno/Windows error の文字列化      | `ket::platform::ErrnoMessage`, `ket::platform::WindowsErrorMessage`, `ket::platform::EnvironmentVariable`     | Windows API は `_WIN32` 限定、env missing は空    | known errno、missing env、Windows guard    | Windows は UTF-8 変換            |
| `state`          | P2       | `Ready`              | C++17   | C++17以降 | なし       | header-only + test               | state       | 小さい状態遷移表                    | `ket::state::Transition`, `ket::state::IsValidTransition`, `ket::state::NextState`                            | table先頭一致、未定義遷移は失敗                   | known/unknown、duplicate、enum             | FSM frameworkなし                |
...
| `math`           | P2       | `Ready`              | C++11   | C++11以降 | なし       | header-only + test               | math        | 補間・角度・単位変換                | `ket::math::Lerp`, `ket::math::NearlyEqual`, `ket::math::TryKiBToBytes`                                       | 浮動小数点演算は FP型限定、byte変換はoverflow失敗 | endpoints、epsilon、large values           | units frameworkなし              |
...
| `color`          | P3       | `Ready`              | C++11   | C++11以降 | なし       | header-only + test               | value       | RGB小値型                           | `ket::color::Rgb`, `ket::color::TryParse`, `ket::color::FormatHex`                                            | C++11なので Try-parse、alpha なし                 | hex長、不正文字、format                    | 6桁hex、先頭 # 任意              |
| `percent`        | P3       | `Ready`              | C++11   | C++11以降 | なし       | header-only + test               | value       | percent小値型                       | `ket::percent::Percent`, `ket::percent::TryFromRatio`, `ket::percent::Clamp`                                  | basis points、範囲外/NaN は失敗、clamp は0%       | 0/100、負値、>100、ratio、rounding         | 内部表現は0..10000               |
...
- Failure/edge cases: nibble > 9、`nullptr`、空入力、非10進数字、整数 overflow、負数、
  固定幅桁数超過は失敗値。`ToDecimalString` は既存実装どおり `data == nullptr` または `size == 0`
  を `std::nullopt` として扱い、空文字列は返さない。
- Complexity/performance: 固定幅 `ToInt`/`FromInt` は定数時間、`constexpr` でコンパイル時評価可。
  `ToDecimalString`/`FromDecimalString` は入力長 O(n)で、戻り値の生成に allocation 1回。
...
- Public API Signatures（`namespace ket::hex`）:
  ...
  - `std::string EncodeU64(std::uint64_t value, unsigned min_width = 0, LetterCase letter_case = LetterCase::kUpper)`
...
- Complexity/performance: `Encode`/`Dump` は入力長 O(n)で出力 string を 1回確保。`Decode` は
  O(n)で vector を 1回確保。`EncodeU64` は桁数分の O(1)。
...
- Behavior: 成功条件は完全消費。`TryInt` は signed integral、`TryUInt` は unsigned
  integral。`TryHex` は `0x`/`0X` prefix あり/なし両方を許可する。
...
- Public API Signatures（`namespace ket::bytes_builder`）:
  - `class Builder`:
    ...
    - `Builder& StringAscii(std::string_view text);`
...
- Public API Signatures（`namespace ket::byte_view`）:
  - `class View`:
    ...
    - `bool TrySubView(std::size_t offset, std::size_t count, View& out) const noexcept;`
  - `class MutableView`:
    ...
    - `bool TrySubView(std::size_t offset, std::size_t count, MutableView& out) const noexcept;`
  - `View MakeView(const std::uint8_t* data, std::size_t size) noexcept;`
  - `MutableView MakeMutableView(std::uint8_t* data, std::size_t size) noexcept;`
...
- Behavior: `TryReadExactly` は requested size を読み切った場合のみ `true`。行読みは末尾の ASCII
  whitespace を除去する。`TryReadLineTrimmed` は成功時だけ `out` を更新し、C++17 convenience は初回
  API に含めない。`StateSaver` は flags、precision、fill を保存・復元する。copy/move はともに禁止し、
...
- Complexity/performance: read/write は size O(n)。行読みは行長 O(n)。`StateSaver` は O(1)。
...
- Do not implement: `std::format` 再実装、logging framework、locale対応、`EncodeU64`（hex module）、
...
- Behavior: pointer alignment と byte表現の読み取りに限定する。`alignment` は power-of-two 前提で、それ以外や
  `alignment == 0` は false。`SecureZero` は `volatile unsigned char*` 経由で byte write し、最適化除去を
...
- Failure/edge cases: `alignment == 0`、非power-of-two alignment、null pointer、secure zero最適化除去対策。
  `Zero(nullptr, 0)` と `SecureZero(nullptr, 0)` は no-op、`nullptr` かつ非0 size は precondition 違反。
...
- Behavior: dotted decimal のみ。leading zero は失敗。`ToBe32` は network byte order の整数
  表現。型名は Google C++ Style に合わせ `ket::ipv4::Address`（proposal の `IpV4` ではない）を採用する。
...
- Public API Signatures（`namespace ket::optional`）:
  ...
  auto Map(const std::optional<T>& value, F f)
      -> std::optional<std::decay_t<decltype(f(*value))>>;

  template <typename T, typename F>
  auto Map(std::optional<T>&& value, F f)
...
- Behavior: empty の場合は mapper/factory を呼ばない。`Map` の mapper 戻り型は non-void object
...
- Public API Signatures（`namespace ket::tlv`）:
  ...
  - `std::vector<std::uint8_t> Encode(std::uint16_t type, const std::uint8_t* value, std::uint32_t value_size);`
  - `void Append(std::vector<std::uint8_t>& dst, std::uint16_t type, const std::uint8_t* value, std::uint32_t value_size);`
...
- Failure/edge cases: `value == nullptr && value_size == 0` は空 value。`value == nullptr && value_size > 0` は
  encode 側 precondition 違反。`TryDecode(nullptr, 0, out)` は `false` で `out` 不変。
  `TryDecode(nullptr, size > 0, out)` は precondition 違反。入力が6 bytes未満、declared length が残り size を
...
- C++ version: 最小要件 C++11。推奨版 C++11以降。推奨理由:
  ...
  非推奨版 なし。非推奨理由: なし。
- Drop-in files: `modules/math/ket_math.h`,
...
- Public API Signatures（`namespace ket::color`）:
  - `struct Rgb { std::uint8_t r = 0; std::uint8_t g = 0; std::uint8_t b = 0; };`
  - `bool TryParse(const std::string& text, Rgb& out) noexcept;`
  - `std::string FormatHex(Rgb color, bool with_hash = true);`
- Behavior: 6桁hex `RRGGBB` を parse し、先頭 `#` は任意。alpha は扱わない。C++11 最小なので入力は
  ...
  `FormatHex` は lower-case、`with_hash` で `#` 前置を制御する。
