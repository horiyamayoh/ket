...
| `io_stream`      | P1       | `Existing`           | C++11   | C++11以降 | なし       | `.h` + `.cpp` + test             | stream      | stream の確実な読み書き                  | `ket::io_stream::TryReadExactly`, `ket::io_stream::TryWriteAll`, `ket::io_stream::StateSaver`                 | requested size を読み切った時だけ成功                   | short read、state復元、line trim           | ASCII trim のみ                  |
...
- Behavior: `TryReadExactly` は requested size を読み切った場合のみ `true`。`TryWriteAll` は requested
  ...
  除去し、成功時だけ `out` を更新する。C++17 convenience は初回 API に含めない。`StateSaver` は
...
- Failure/edge cases: short read/write、stream error、stream exception、`data == nullptr && size > 0`
  はstreamへアクセスせず`false`。EOF で1文字も読めない場合は行読み失敗で `out` 不変。空行を
  読めた場合と、改行なしでEOFへ到達した最終行は成功。
- Complexity/performance: read/write は size O(n)。行読みは行長 O(n)。`StateSaver` は O(1)。
- Tests: exact read、short read、null buffer、stream error、exception propagation、state復元、trimmed
  line、EOF。
