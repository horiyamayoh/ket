...
`wire` は ordinary module ではなく package runtime である。`wire` の公開API、配置、失敗規約、
lifetime policy、実装 owner、検証条件の正本は
[`docs/packages/ket_wire_design.md`](packages/ket_wire_design.md) と
`packages/wire/ket_wire.h` の Doxygen とし、この文書では一覧上の登録と module/package 境界だけを扱う。
...
- 公開APIは module ごとの入れ子 `namespace ket::<module>` に置く。C++11/14 の drop-in を維持するため `namespace ket { namespace <module> { ... } }` の入れ子 block 形式で書き、C++17 短縮形は使わない。top-level の `namespace ket` は1つだけ。macro-only module だけは C++ namespace に置けないため、`KET_` prefix の global macro に限定し、内部 helper macro は `KET_DETAIL_` prefix にする。内部 helper は header 内なら `ket::<module>::detail`、
...
`progress.md` で `implemented` または `verified` の module/package は、この一覧では
`Existing` として扱う。`Ready` は未実装だが製造依頼可能な仕様カードに限定する。
...
| `bits`           | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | numeric     | bit/nibble/mask の事故防止                   | `ket::bits::HighNibble`, `ket::bits::LowNibble`, `ket::bits::TryPackNibbles`, `ket::bits::TryMask`                                               | unsigned integral 限定、char系と範囲外 bit は失敗/false     | nibble、bit幅、mask境界                                          | C++20 `<bit>` は一部のみ重複     |
| `numeric`        | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | numeric     | overflow、align、cast の小さい正解           | `ket::numeric::TryAlignUp`, `ket::numeric::TryAdd`, `ket::numeric::TryCast`                                                                      | 0除算、alignment 0、overflow は失敗                         | min/max、overflow、signed/unsigned                               | optional convenience は初回なし  |
| `endian`         | done     | `Existing`           | C++11   | C++11以降 | なし       | `.h` + `.cpp` + test             | binary      | unaligned/endian 読み書き                    | `ket::endian::LoadBe32`, `ket::endian::LoadLe32`, `ket::endian::StoreBe16`, `ket::endian::TryLoadBe32`                                           | plain load/store は precondition、Try は失敗値              | BE/LE、null、size不足                                            | reinterpret cast 禁止            |
| `hex`            | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | diagnostic  | bytes と16進文字列/hex dump                  | `ket::hex::Encode`, `ket::hex::Decode`, `ket::hex::Dump`                                                                                         | null+非0 size は precondition、奇数桁は失敗                 | 空入力、separator、不正文字                                      | dump形式は固定                   |
...
| `enums`          | done     | `Existing`           | C++17   | C++17以降 | なし       | header-only + test               | enum        | enum class と文字列変換                      | `ket::enums::Entry`, `ket::enums::Name`, `ket::enums::Parse`, `ket::enums::HasFlag`                                                              | table完全一致、重複は先勝ち                                 | known/unknown、duplicate、flags                                  | reflection はしない              |
| `container`      | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | container   | map/vector の小さい儀式                      | `ket::container::Contains`, `ket::container::ContainsKey`, `ket::container::AtOrNull`, `ket::container::AtOr`                                    | 見つからない場合は null/default/0                           | key有無、factory呼び出し、削除件数                               | `IndexOf`/`Append` は初回外      |
...
| `byte_reader`    | done     | `Existing`           | C++11   | C++11以降 | なし       | `.h` + `.cpp` + test             | binary      | byte列の安全な逐次読み取り                   | `ket::byte_reader::Reader`, `ket::byte_reader::Reader::ReadU8`, `ket::byte_reader::Reader::ReadBytes`                                            | 成功時だけ offset 更新、invalid reader は失敗               | 空、ぴったり、size不足、offset保持                               | endian module 非依存             |
...
| `bytes`          | done     | `Existing`           | C++17   | C++17以降 | なし       | header-only + test               | binary      | owning payload builder                       | `ket::bytes::Builder`, `ket::bytes::Builder::AppendU8`, `ket::bytes::AppendBe16`                                                                 | allocation例外あり、null+非0 size は precondition           | BE/LE、reserve、Build後move                                      | fluent と free を併用            |
| `date`           | done     | `Existing`           | C++11   | C++11〜17 | C++20以降  | header-only + test               | date        | 日付・時刻の妥当性                           | `ket::date::IsLeapYear`, `ket::date::IsValidDateTime`, `ket::date::TryGetDaysInMonth`                                                            | Gregorian、year >= 1、leap secondなし                       | 2000/1900、2/29、month 0/13                                      | C++20以降は `std::chrono` を優先 |
| `deadline`       | done     | `Existing`           | C++11   | C++11以降 | なし       | `.h` + `.cpp` + test             | time        | timeout と elapsed time                      | `ket::deadline::Stopwatch`, `ket::deadline::Deadline`                                                                                            | `steady_clock`のみ、負timeoutは期限切れ                     | zero/future/remaining/restart                                    | system_clock と混ぜない          |
...
| `byte_view`      | done     | `Existing`           | C++11   | C++11〜17 | C++20以降  | header-only + test               | view        | non-owning byte span                         | `ket::byte_view::View`, `ket::byte_view::MutableView`, `ket::byte_view::View::TrySlice`                                                          | `nullptr+0` は空、`nullptr+非0` は invalid                  | lifetime、slice、bounds                                          | constructor を使う               |
...
| `mac`            | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | network     | MAC address parse/format                     | `ket::mac::Address`, `ket::mac::operator==`, `ket::mac::Parse`, `ket::mac::Format`                                                               | `:` と `-` を許可、混在とCisco形式は失敗                    | upper/lower、不正hex、区切り、比較                               | Cisco形式なし                    |
| `function`       | done     | `Existing`           | C++17   | C++17以降 | なし       | header-only + test               | callable    | callable/visitor の儀式除去                  | `ket::function::Overload`, `ket::function::MakeOverload`, `ket::function::Noop`                                                                  | 空/非class callable は compile error、`Noop` は例外なし     | visit、overload解決、decay、copy/move                            | `FunctionRef` は初回なし         |
...
| `platform`       | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | platform    | errno/Windows error/env境界                  | `ket::platform::FormatErrno`, `ket::platform::FormatWindowsError`, `ket::platform::ReadEnvironmentVariable`                                      | Windows API は `_WIN32` 限定、env missing は `std::nullopt` | known errno、missing/empty env、UTF-8 failure, non-Windows smoke | Windows は UTF-8 変換            |
...
| `cache`          | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | cache       | once/lazy value                              | `ket::cache::Lazy<T>`, `ket::cache::Lazy<T>::HasValue`, `ket::cache::Lazy<T>::GetOrCreate`                                                       | non-thread-safe、例外後は空                                 | factory回数、reset、例外後状態                                   | thread-safe にしない             |
...
| `wire`           | done     | `Existing`           | C++17   | C++17以降 | なし       | package header + test            | package     | schema-based binary wire runtime             | `ket::wire::MakeSchema`, `ket::wire::DecodeExact`, `ket::wire::Encode`, `ket::wire::EncodeTo`                                                    | `Status`で失敗理由を保持、decode/encode失敗は失敗値         | 実装済み境界値テスト                                             | module-first package runtime     |
| `tuple`          | done     | `Existing`           | C++17   | C++17以降 | なし       | header-only + test               | tuple       | tuple/pair の小さい補助                      | `ket::tuple::ForEach`, `ket::tuple::Transform`                                                                                                   | evaluation order と戻り型を固定                             | empty、heterogeneous、const                                      | structured binding 競合は避ける  |
| `build_config`   | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | config      | feature detection                            | `KET_CXX_VERSION`, `KET_CXX_AT_LEAST`, `KET_HAS_STD_OPTIONAL`, `KET_OS_LINUX`                                                                    | macro は `KET_` prefix、値は 0/1                            | compiler/OS 条件、include順、C++11/14/20/23                      | 他moduleの必須依存にしない       |
...
| `meta`           | done     | `Existing`           | C++11   | C++11以降 | API別      | header-only + test               | meta        | type traits 補助                             | `ket::meta::RemoveCvref`, `ket::meta::TypeIdentity`, `ket::meta::VoidT`                                                                          | alias のみ、評価時動作なし                                  | alias、SFINAE、C++11 compile                                     | module単位では非推奨にしない     |
| `concurrency`    | done     | `Existing`           | C++11   | C++11以降 | API別      | header-only + test               | concurrency | join/future readyの局所補助                  | `ket::concurrency::JoiningThread`, `ket::concurrency::JoiningThread::Joinable`, `ket::concurrency::IsReady`                                      | dtorでjoin、move代入で旧threadをjoin                        | move、joinable、ready/deferred                                   | module単位では非推奨にしない     |
| `uuid`           | done     | `Existing`           | C++17   | C++17以降 | なし       | `.h` + `.cpp` + test             | parsing     | UUID parse/format                            | `ket::uuid::Uuid`, `ket::uuid::Parse`, `ket::uuid::Format`                                                                                       | canonical hyphen形式のみ、generationなし                    | upper/lower、不正長、不正hex、roundtrip                          | 乱数/OS APIなし                  |
...
| `percent`        | done     | `Existing`           | C++11   | C++11以降 | なし       | header-only + test               | value       | percent小値型                                | `ket::percent::Percent`, `ket::percent::Percent::TryFromRatio`, `ket::percent::Percent::FromPercentClamped`                                      | basis points、範囲外/NaN は失敗、clamp は0%                 | 0/100、負値、>100、ratio、rounding                               | 内部表現は0..10000               |
...
- Public API Signatures（`namespace ket::bcd`）:
  ...
  - `template <typename Packed> constexpr int MaxInt() noexcept`
...
- Behavior: 固定幅 packed BCD は整数へ変換し、任意バイト長 packed BCD は桁数と先頭ゼロを
  ...
  `Validate` は invalid storage、byte offset、actual nibble、high/low nibble位置を返す。
- Failure/edge cases: nibble > 9、`nullptr + 非0 size`、非10進数字、整数 overflow、負数、
  固定幅桁数超過は失敗値。`Validate(nullptr, 0)` は valid empty、`Validate(nullptr, size > 0)` は
  `ValidationError::kInvalidStorage`。`Format` は既存実装どおり `data == nullptr` または
  `size == 0` を `std::nullopt` として扱い、空文字列は返さない。`Format` / `Parse` は任意長入力を
  線形走査し、戻り値生成で allocation するため、untrusted input では caller 側で最大byte数または
  最大文字数を確認してから呼ぶ。
- Complexity/performance: 固定幅 `ToInt`/`FromInt` は定数時間、`constexpr` でコンパイル時評価可。
  `Format`/`Parse` は入力長 O(n)で、戻り値の生成に allocation 1回。
- Tests: 0x00、0x09、0x10、0x99、不正nibble、invalid storage、byte offset、actual nibble、
  high/low nibble、空入力失敗、先頭ゼロ保持、奇数桁文字列、非数字、`MaxInt<Packed>`。
- Do not implement: `BcdDate`、`BcdTime`、業務固有BCD解釈。妥当性判定は
  `IsBcdByte`/`IsBcd16`/`IsBcd32`/`Validate` の byte/word/byte列単位に留め、nibble単体判定の公開APIは追加しない。
...
- Public API Signatures（`namespace ket::bytes`）:
  ...
  - `void AppendBe64(std::vector<std::uint8_t>& dst, std::uint64_t value);`
  ...
  - `void AppendLe64(std::vector<std::uint8_t>& dst, std::uint64_t value);`
  - `void AppendFill(std::vector<std::uint8_t>& dst, std::uint8_t value, std::size_t count);`
...
- Failure/edge cases: allocation があるため `noexcept` なし。`std::vector::max_size()`超過は
  `std::length_error` を送出し、対象bufferを変更しない。固定幅 append（`AppendBe16/Be32/Be64/Le16/Le32/Le64`）は
  一時配列を単一 insert で追記し、allocation 失敗時は `dst` を変更しない strong exception guarantee。
  `AppendFill(value, 0)` は no-op。
...
- Tests: U8/BE/LE 16/32/64 append、fill count 0/非0、reserve constructor、Clear、Buffer、Build move、
  null+0、ASCII append。
...
- Behavior: view は所有権を持たず、元 buffer lifetime に依存する。`TryAt`/`TrySlice` は範囲内のみ
  ...
  `MutableView::TrySet` は範囲内のみ書き込む。free helper は `TrySlice` の事前確認や
  cursor-based runtime の残byte計算に使える constexpr API。
- Failure/edge cases: `nullptr+0` は空 view。`nullptr+非0` は invalid view とし、access は失敗。
  slice 範囲超過は失敗で out 不変。`Remaining` は invalid view または `offset > Size()` で 0 を返す。
- Complexity/performance: 全 method は O(1)。view は pointer と size のみ保持し allocation なし。
- Tests: default、empty、invalid、bounds、slice、mutable set、free helper constexpr/static_assert、
...
- Purpose: MAC address の parse/format。Existing module のため、公開詳細は header Doxygen と実装を正本とし、
...
- Behavior: `AA:BB:CC:DD:EE:FF` と `aa-bb-cc-dd-ee-ff` を許可する。区切り文字の混在は失敗。
  `Address` の比較は6 byteすべての値で行う。`Format` は `letter_case` で大文字小文字を選び（既定は lower-case）、
  `:` 区切りで出力する。
- Failure/edge cases: byte数不足/過多、不正hex、不正separator、区切り不足、混在区切り、Cisco形式は失敗。
- Complexity/performance: parse/format は固定6 byte の O(1)。allocation は format のみ。
- Tests: upper/lower、colon、hyphen、format lower/upper、比較、不正hex、不正separator、短い/長い入力。
- Do not implement: Cisco形式、OUI lookup、network device model。

### function Module

- Purpose: callable/visitor の小さい儀式を減らす。Existing module のため、公開詳細は header Doxygen と実装を
  正本とし、この文書は採用範囲の要約を記録する。
- C++ version: 最小要件 C++17。推奨版 C++17以降。推奨理由:
...
- Public API Signatures（`namespace ket::function`）:
  ...
  - `template <typename... Fs> constexpr Overload<std::decay_t<Fs>...> MakeOverload(Fs&&... fs) noexcept(...);`
  - `struct Noop { template <typename... Args> constexpr void operator()(Args&&...) const noexcept; };`
- Behavior: `Overload` は1個以上の callable class を値として保持し、`using Fs::operator()...` で overload set
  を作る。C++17 CTAD 用 deduction guide を持つ。`MakeOverload` は lvalue callable をcopy、rvalue callableをmoveし、
  decay後の型で所有する。構築がnoexceptなら `MakeOverload` もnoexcept。`Noop` は任意引数を無視し
  `constexpr` / `noexcept`。
- Failure/edge cases: 空の `Overload`、非class型の callable、成立しない overload set は compile error。
  callable の copy/move 制約は型に従う。呼び出し例外は handler から伝播。
- Complexity/performance: `Overload` は callable を値合成するのみで O(1)、追加 allocation なし。
- Tests: `std::visit`、複数型、戻り値、Noop `constexpr`/`noexcept`、deduction guide、decay、copy/move。
...
- Behavior: `FormatErrno` は errno 番号を platform の thread-safe API で文字列化し、結果を
  ...
  ASCII fallback を返す。`ReadEnvironmentVariable` は process environment から値を取得し、存在しなければ
  ...
  signature 差は `.cpp` 内 helper に閉じ込め、公開APIの戻り値や例外方針に差を出さない。空値の environment
  variable は missing と区別し、空の `std::string` として返す。
- Failure/edge cases: unknown errno、missing env、empty name、NUL を含む name、Windows wide/narrow 変換失敗。
  environment を他 thread が同時変更する場合の一貫性は platform API の規約に従う。Windows env 変換に失敗した
  場合は `std::nullopt`、Windows error message の UTF-8 変換に失敗した場合は ASCII fallback を返す。
- Complexity/performance: error message と environment value の長さ O(n) で string を1回確保する。
  Windows UTF-8 変換も出力長 O(n)。
- Tests: known errno が非空、unknown errno fallback、errno preservation、missing env、present env、empty env、
  empty name、NUL name、Windows env name/value UTF-8 conversion failure、Windows error message UTF-8 conversion
  fallback、non-Windows で Windows API が宣言されない conditional compile、POSIX/GNU strerror差の compile、
  Windows 環境では known `GetLastError` message と last-error preservation。
  environment test は一意な `KET_` prefix の変数名を使い、test 前の値を保存して終了時に復元する。
...
- C++ version: 最小要件 C++11。推奨版 C++11以降。推奨理由:
  lazy value の非 thread-safe 方針と例外後状態を局所的に固定できる。
  非推奨版 なし。非推奨理由: なし。
...
- Behavior: non-thread-safe。C++11 で `alignas(T)` 付きの byte storage による手動 storage を使い、
  ...
  参照を返す。factory object は `Lazy` 内へ保持せず、lvalue/rvalue をそのまま呼び出す。`Reset` と destructor は
  保持値がある場合だけ破棄して empty に戻す。factory または `T` 構築が例外を投げた場合は empty のまま。
  `Lazy<T>` の object address は `Reset` しない限り保持値の lifetime 中に変わらない。
- Failure/edge cases: factory例外後は empty、Reset後は再生成、move-only value、再入は precondition 違反。
  ...
  要求し、破棄中の例外は `std::terminate`。`GetOrCreate` 中または `Reset` 中に同じ `Lazy` の
  `GetOrCreate`/`Reset` を呼ぶ再入は precondition 違反であり、検出時は `std::terminate`。
- Complexity/performance: `HasValue`/`Reset` と生成後の `GetOrCreate` は O(1)。保持値がある間は factory を呼び出さず、
  生成サイクルごとに必要時だけ1回呼び出す。
  storage は object 内に置くため追加 allocation なし。
- Tests: factory呼び出し回数、Reset、例外後状態、move-only value、保持値 address stability、
  copy不可 lvalue factory、copy/move 禁止 compile-only、destructor 例外時 terminate、再入時 terminate。
...
- Purpose: compiler、OS、standard library feature の小さい判定 macro。Existing module のため、公開詳細は
  header Doxygen と実装を正本とし、このカードは採用済み仕様の要約として保守する。
- C++ version: 最小要件 C++11。推奨版 C++11以降。推奨理由:
...
- Dependencies: 標準ライブラリ、compiler predefined macros、Apple platform では `TargetConditionals.h`。
  他の ket module への依存なし。
...
- Behavior: 全 macro は `KET_` prefix とし、feature/compiler/OS 判定 macro の値は必ず `0` または `1`。
  ...
  C++23超の標準値は compiler が提供する値を保持し、`202302L` へ丸めない。
  ...
  MSVC を `_MSC_VER` で判定し、clang-cl や既知の互換 compiler では重複を避ける。
  OS 判定は Windows を `_WIN32`、Linux を `__linux__ && !__ANDROID__`、macOS を
  `TargetConditionals.h` の macOS target で判定し、iOS、tvOS、watchOS、Mac Catalyst、unknown OS は対象外。
  unknown OS は3 macro すべて 0 とする。macOS macro は `KET_OS_MACOS` を採用し、
  ...
  feature macro は 0 に倒す。header が存在しても feature-test macro が無い場合は 0。
...
- Tests: C++11/14/20/23 compile-only、macro が全て定義済み、0/1 値、
  `KET_CXX_AT_LEAST` 境界、compiler/OS macro の条件、clang/GCC/MSVC の相互排他条件、
  unknown OS が全0でも成立すること、optional/string_view/span/format header availability と feature-test macro
  に応じた値、単独 include と標準 header include 後の include 順確認。
...
- Purpose: join 忘れと future ready 確認程度の局所補助。Existing module のため、公開詳細は
  header Doxygen と実装を正本とし、このカードは採用済み仕様の要約として保守する。
- C++ version: 最小要件 C++11。推奨版 C++11以降。推奨理由:
...
- Behavior: `JoiningThread` は所有する `std::thread` を destructor で joinable なら join する。move 代入は
  ...
  内部 `std::thread` の直接操作APIは公開せず、RAII joinの責務を `JoiningThread` に閉じる。
- Failure/edge cases: self-join は precondition 違反。destructor や move代入中に join できない状況、または
  ...
  `IsReady` は `future.valid()` を precondition とし、invalid future は呼び出さない。future-like object の
  `wait_for` が例外を投げる場合、その例外は呼び出し元へ伝播する。compile-only check では `std::future` と
  `std::shared_future` の両方で `wait_for(0)` が使えることを確認する。
- Complexity/performance: `Joinable`/`IsReady` は O(1)。destructor と move 代入は join 完了まで blocking。
- Tests: default、joinable、move/self-move、move代入時の旧thread join、ready/not ready、deferred、
  invalid future precondition の Doxygen 明記、const future、非joinable thread、C++11 compile-only。
- API別標準代替:
...
- Public API Signatures（`namespace ket::uuid`）:
  - `struct Uuid { std::array<std::uint8_t, 16U> bytes = {}; };`
...
- Failure/edge cases: 長さ不一致、hyphen位置不一致、不正hex、brace/URN形式は失敗。
- Complexity/performance: parse/format は固定36文字/16 byte の O(1)。allocation は format のみ。
- Tests: zero uuid、normal uuid、all-ff uuid、upper input、bad length、bad hyphen、bad hex、
  brace/URN rejected、format lower-case、roundtrip、Doxygen例。
...
- Purpose: 0〜100% の小さい値型。Existing module のため、公開詳細は header Doxygen と実装を正本とし、
  このカードは採用済み仕様の要約として保守する。
- C++ version: 最小要件 C++11。推奨版 C++11以降。推奨理由:
...
- Rounding note: `double` 入力は binary floating-point 値として扱い、その値を scale 後に
  `floor(value + 0.5)` 相当で丸める。十進表記の厳密な tie 丸めが必要な箇所では
  `TryFromBasisPoints` を使う。
...
- Tests: default 0%、basis points 0/1/10000/10001/`std::uint32_t`最大値、percent
  0/0.004/0.005/12.345/99.994/99.995/100、ratio 0/0.00004/0.00005/0.12345/1、
  負値、上限直外、NaN、Inf、FromPercentClamped、比較演算、out不変、C++11 compile-only。
