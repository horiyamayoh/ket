...
```cpp
const auto value = ket::LoadBe32(data);
const auto aligned = ket::AlignUp(size, 4U);
const auto mode = ket::ParseEnum<Mode>(text, kModeTable);
const auto name = ket::GetOrDefault(names, id, "unknown");
```
...
ket::BytesBuilder      // owning builder
...
| `testing_bytes`     | P1   | C++17    | bytes系テスト補助                | `ExpectBytesEq`, `ExpectHexEq`                    |
...
| `c_interop`         | P2   | C++11    | C API 境界の事故防止             | `ErrnoGuard`, `CopyToCBuffer`, `UniqueHandle`     |
...
constexpr ket::EnumEntry<Mode> kModeTable[] = {
	{Mode::kAuto, "auto"},
	{Mode::kManual, "manual"},
};

const auto mode = ket::ParseEnum("auto", kModeTable);
```
...
```cpp
ket::TrimAscii(text)
ket::TrimLeftAscii(text)
ket::TrimRightAscii(text)
```

### 11.2 `Bytes` より `BytesBuilder`

`Bytes` は container 型に見える。payload builder なら `BytesBuilder` のほうが意図が明確。

```cpp
ket::BytesBuilder{}.U8(x).Be16(y).Build();
```

### 11.3 `Finally` は最初は不要

`ScopeExit` と `MakeScopeExit` があれば十分。

```cpp
auto guard = ket::MakeScopeExit([&] { cleanup(); });
```
