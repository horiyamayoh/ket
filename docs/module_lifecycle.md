...
package runtime からの要請で既存 module に公開APIを追加する場合も、同じように候補APIとして
痛み、利用箇所、境界条件、テスト観点を残します。
...
- 公開ヘッダでは、公開APIのDoxygen付き宣言、ヘッダ内helperが必要な場合は
  `ket::<module>::detail` の内部helper、ヘッダ内に必要な公開API定義の順に書きます。
...
- ordinary module は原則として他の ket module に依存しません。package runtime は module を合成してよいが、
  module から package へ依存させません。
- package runtime からの要請で公開APIを追加する場合は、package 固有型、package 固有 status、
  protocol 固有語を公開APIへ持ち込まず、module単体でも自然に使える小さい汎用APIにします。
- package-driven API extension は additive change を原則とし、既存APIの破壊的変更は明示的な
  破壊的変更提案なしに行いません。
...
- `@par namespace` には `公開API：ket::<module>` と内部helperの配置を書きます。header内helperがある場合は
  `内部実装：ket::<module>::detail`、`.cpp` 内helperだけの場合は `内部実装：.cpp の無名 namespace`、内部helperがない場合は
  `内部実装：なし` と書きます。
- ヘッダのinclude guardには `#pragma once` を使います。
