...
- `byte_view`: public API の input/output view、view妥当性、残byte数、slice境界確認。
- `byte_reader`: decode 時の field byte 読み取りと offset 保持。
- `byte_writer`: fixed output buffer への field byte 書き込みと offset 保持。
- `endian`: fixed-width integer の byte order 変換。
- `bits`: bit packing、mask、reserved bit の小処理。
- `numeric`: overflow、range、size 計算の境界確認。
- `bcd`: packed BCD の nibble 検証、diagnostics、integer 変換、桁上限。
- `bytes`: owning encode buffer の構築と fill。

package 内だけで必要な処理でも、`wire` 以外で独立して使う価値がある場合は ordinary module として抽出する。
既存 module に十分近い処理がある場合、`wire` は重複実装を避けるために module へ公開API追加を要求してよい。
その API は package 固有型、package 固有 status、protocol 固有語を含めず、module単体でも自然に使える
additive API とする。
...
既存 module の責務に自然に属する処理は、package 内 detail として二重実装せず、module API 追加を検討する。
module API 追加は既存APIを壊さない additive change に限定し、仕様カード、Doxygen、test 観点を同時に更新する。
...
`noexcept` にしない。確保失敗や `std::vector` の上限超過は標準例外として送出され、`EncodeResult` は返らない。
`EncodeTo` は caller が渡した `ket::byte_view::MutableView` へ書く fixed-buffer API であり、成功時だけ先頭
...
1. input `ket::byte_view::View` の valid/invalid を `ket::byte_view::IsValid` で検査する。
2. schema の field 配列、field count、message 名を検査する。
3. temporary object を用意する。
4. field ごとに `ket::byte_view::Remaining` と `ket::byte_reader::Reader::ReadBytes` で入力範囲を確認する。
5. fixed-width integer は `ket::endian::Load*`、BCD は `ket::bcd::Validate` と `ket::bcd::ToInt`、
   grouped bits は `ket::bits::TryMask` を使って decode する。
6. field を順番に decode し、temporary object だけを更新する。
7. exact decode では consumed byte 数が input size と一致したことを検査する。
8. prefix decode では consumed byte 数を計算し、成功 result へ入れる。
9. すべて成功した場合だけ `DecodeResult<T>::value` へ temporary object を move/copy する。
...
- `Encode` の schema/preflight 失敗時は `bytes == std::nullopt`。allocation失敗は標準例外として送出。
- value object は変更しない。
- status は失敗理由を保持する。
...
各 owner は他 owner の変更を revert しない。package-driven API 追加の対象外である既存 binary modules は
read-only reference として扱う。
...
- `Field<T>`、`BitMember<T>`、`Schema<T, N>` は read-only metadata accessor を公開し、
  callback pointer や内部 validation state を利用者が直接組み立てない。
- backend は `byte_view`、`byte_reader`、`byte_writer`、`endian`、`bcd`、`bytes`、`bits`、`numeric`
  を合成し、同等処理を package detail に重複実装していない。
