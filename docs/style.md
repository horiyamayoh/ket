...
- 公開APIは module ごとの入れ子namespace `namespace ket::<module>` に置き、UpperCamelCaseにします。
...
公開APIは module ごとの入れ子namespace `namespace ket::<module>` に置きます。
C++11/14のdrop-inを維持するため、C++17の `namespace ket::bcd { ... }` 短縮形は使わず、入れ子block形式で書きます。
...
- header内で必要な内部helperは `ket::<module>::detail` に置き、`.cpp` 内だけで使うhelperは無名namespaceに置きます。
...
- bufferの固定位置: `Load<Order><Width>` / `Store<Order><Width>`（`LoadBe32` など。widthはmodule内variantのため残す）
- statefulなcursorのmember: `Read*` / `Write*`（`std::istream` 流の素の動詞。boolを返してもTryは付けない）
...
1. 公開APIのDoxygen付き宣言
2. header内helperが必要な場合は `ket::<module>::detail` の内部helper
...
- header内で必要なhelperは `ket::<module>::detail` に置きます。
...
- namespace終端コメントは `// namespace bcd`、`// namespace ket` のように標準的な英語表記にします。入れ子namespaceは内側から順に閉じ、それぞれに終端コメントを付けます。
...
標準代替や標準に近い機能との差分がmodule採用判断に有用な場合は、`非推奨理由：なし。` の後へ任意の `標準代替：...` を追加してよいです。
ただし、`標準代替` は非推奨理由に混ぜず、C++バージョン単位の非推奨有無と分けて書きます。
...
`.cpp` 内だけにhelperを置くmoduleでは、実体と一致するように `内部実装：.cpp の無名 namespace`
と書きます。
内部helperを持たないmoduleでは、実体と一致するように `内部実装：なし` と書きます。
...
- `@retval`（constructorとdestructor以外）
- `@pre`
- `@post`
- 公開API関数では `@code` と `@endcode`

constructorとdestructorは戻り値を持たないため、`@retval` と `@return` を書きません。
copy constructor、move constructorも同じ扱いです。
生成後または破棄後の性質は `@post` に書きます。
constructorに引数がある場合は、通常の関数と同じく `@param[in]` を書きます。

必要に応じて `@note`、`@remark` を追加します。
公開API関数の `@code` は、入力例、戻り値、失敗時の形が一目で分かる短い例にします。
`ket::<module>::detail` の内部helperでは、必要な場合だけ `@code` を追加します。
...
`ket::<module>::detail` 配下の内部型でも、ヘッダに置く場合は読み手が役割を追える短い説明を残します。
