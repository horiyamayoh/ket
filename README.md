...
- module/API製造依頼用の正本は [docs/module_api_catalog.md](docs/module_api_catalog.md)
  で管理します。
- ブラウザで俯瞰する静的リファレンスは [site/index.html](site/index.html) から閲覧できます。
  `file://` で開ける生成済みHTMLです。
...
`site/` は生成済みHTMLです。手編集せず、正本である `modules/*/ket_*.h` のDoxygen、
`progress.md`、`docs/site_src/` を更新してから再生成します。
...
生成と検査はCMake targetからも実行できます。

```sh
cmake --build --preset dev --target generate-site
cmake --build --preset dev --target check-site
```
